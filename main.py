"""
FastAPI Web Content Parser and QA Assistant
"""
    ...
    try:
        ...
        return ProcessResponse(
            ...
            metadata={
                ...
                "search_type": relevant_chunks[0].get("search_type", "semantic") if relevant_chunks else "none"
            }
        )
...
async def parse_html_only(html: str):
    """
    Parse HTML without chunking or semantic search
    
    Args:
        html: Raw HTML content
        
    Returns:
        Parsed content with title and clean markdown
    """
    try:
        result = html_parser.extract_main_content(html)
        
        if not result["success"]:
            raise HTTPException(
                status_code=400,
                detail=f"Failed to parse HTML: {result.get('error', 'Unknown error')}"
            )
